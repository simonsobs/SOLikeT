...
for ibin in np.arange(1, nbins + 1):
    ...
    Nell_bin = (np.ones(nell_win) * sigma_e[ibin - 1] ** 2. / 
        (ngal[ibin - 1] * (1.08e4 / np.pi) ** 2))

    spectra[ibin, 0, :] = ccl.angular_cl(cosmo, shear_tracers[ibin - 1], 
                                         tracer_so_k, ells_win)
    ...
    for jbin in np.arange(1, nbins + 1):
        if ibin == jbin:
            spectra[ibin, ibin, :] = ccl.angular_cl(cosmo, 
                                                    shear_tracers[ibin - 1], 
                                                    shear_tracers[ibin - 1], 
                                                    ells_win) + Nell_bin
            spectra_label[ibin, ibin] = f'g{ibin}g{ibin}'
        else:
            spectra[ibin, jbin, :] = ccl.angular_cl(cosmo, 
                                                    shear_tracers[ibin - 1], 
                                                    shear_tracers[jbin - 1], 
                                                    ells_win)
            spectra[jbin, ibin, :] = ccl.angular_cl(cosmo, 
                                                    shear_tracers[jbin - 1], 
                                                    shear_tracers[ibin - 1], 
                                                    ells_win)
    ...
    for i2 in range(i1, n_maps):
        ...
        for j1 in range(n_maps):
            for j2 in range(j1, n_maps):
                ...
                print(f'cov({id_i}, {id_j}): ({cl_i1j1_lab} * {cl_i2j2_lab} + \
...
keep_spectra = (('gs_des_bin1', 'ck_so'), ('gs_des_bin2', 'ck_so'), \
                ('gs_des_bin3', 'ck_so'), ('gs_des_bin4', 'ck_so'))
