...
class mm_gg_mg_spectra:
    ...
    def halo_terms_matter(self):
        ...
        for k in range(len(self.k_array)):
            ...
            Pk_2h[k, :] = (self.Pk_array[:, k] * 
                           (trapz(intmass_2h[k, :, :], self.mass, axis=-1) / 
                            rho_mean + B) ** 2) 
    ...
    def halo_terms_galaxy(self):
        if self.gal_mod == True:
            intmass_2h_EP = np.zeros([len(self.k_array), 
                                      len(self.redshift), 
                                      len(self.mass)])
            intmass_1h_EP = np.zeros([len(self.k_array), 
                                      len(self.redshift), 
                                      len(self.mass)]) 
            intmass_2h_LP = np.zeros([len(self.k_array), 
                                      len(self.redshift), 
                                      len(self.mass)]) 
            intmass_1h_LP = np.zeros([len(self.k_array), 
                                      len(self.redshift), 
                                      len(self.mass)]) 
            intmass_1h_mix = np.zeros([len(self.k_array), 
                                       len(self.redshift), 
                                       len(self.mass)]) 
            ...
            for k in range(len(self.k_array)):
                intmass_1h_EP[k, :, :] = (self.instance_200.dndM * 
                                          (2 * self.instance_HOD.Ncent_EP[np.newaxis, :] 
                ...
                intmass_1h_LP[k, :, :] = (self.instance_200.dndM * 
                                          (2 * self.instance_HOD.Ncent_LP[np.newaxis, :] 
            ...
            for k in range(len(self.k_array)):
                ...
                intmass_2h[k, :, :] = (self.instance_200.dndM 
                                       * self.instance_200.bias_cib *
                                       self.instance_HOD.Nbra_EP[np.newaxis, :] 
                                       * self.instance_200.u_k[:, :, k])
    ...
    def halo_terms_matter_galaxy(self):
        if self.gal_mod == True:
            ...
            intmass_2h_m = np.zeros([len(self.k_array), 
                                     len(self.redshift), 
                                     len(self.mass)])
            ...
            intmass_1h_EP = np.zeros([len(self.k_array), 
                                      len(self.redshift), 
                                      len(self.mass)])
            intmass_2h_g_EP = np.zeros([len(self.k_array), 
                                        len(self.redshift), 
                                        len(self.mass)])
            ...
            intmass_1h_LP = np.zeros([len(self.k_array), 
                                      len(self.redshift), 
                                      len(self.mass)])
            intmass_2h_g_LP = np.zeros([len(self.k_array), 
                                        len(self.redshift), 
                                        len(self.mass)])
            ...
            intmass_1h = np.zeros([len(self.k_array), 
                                   len(self.redshift), 
                                   len(self.mass)])
            intmass_2h_m = np.zeros([len(self.k_array), 
                                     len(self.redshift), 
                                     len(self.mass)])
            intmass_2h_g = np.zeros([len(self.k_array), 
                                     len(self.redshift), 
                                     len(self.mass)])
