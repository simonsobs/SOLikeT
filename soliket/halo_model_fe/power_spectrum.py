...
class mm_gg_mg_spectra:
    ...
    def halo_terms_matter(self):
        ...
        for k in range(len(self.k_array)):
            ...
            Pk_2h[k, :] = (
                self.Pk_array[:, k]
                * (trapz(intmass_2h[k, :, :], self.mass, axis=-1) / rho_mean + B) ** 2
            )
    ...
    def halo_terms_galaxy(self):
        if self.gal_mod == True:
            intmass_2h_EP = np.zeros(
                [len(self.k_array), len(self.redshift), len(self.mass)]
            )
            intmass_1h_EP = np.zeros(
                [len(self.k_array), len(self.redshift), len(self.mass)]
            )
            intmass_2h_LP = np.zeros(
                [len(self.k_array), len(self.redshift), len(self.mass)]
            )
            intmass_1h_LP = np.zeros(
                [len(self.k_array), len(self.redshift), len(self.mass)]
            )
            intmass_1h_mix = np.zeros(
                [len(self.k_array), len(self.redshift), len(self.mass)]
            )
            ...
            for k in range(len(self.k_array)):
                ...
                Pk_1h_EP[k, :] = (
                    trapz(intmass_1h_EP[k, :, :], self.mass, axis=-1)
                    / self.instance_HOD.ngal_EP_200c**2
                )
                Pk_1h_LP[k, :] = (
                    trapz(intmass_1h_LP[k, :, :], self.mass, axis=-1)
            ...
            for k in range(len(self.k_array)):
                ...
                Pk_1h[k, :] = (
                    trapz(intmass_1h[k, :, :], self.mass, axis=-1)
                    / self.instance_HOD.ngal_EP_200c**2
                )
                Pk_2h[k, :] = (
                    self.Pk_array[:, k]
    ...
    def halo_terms_matter_galaxy(self):
        if self.gal_mod == True:
            ...
            for k in range(len(self.k_array)):
                ...
                Pk_2h_EP[k, :] = (
                    self.Pk_array[:, k]
                    * (trapz(intmass_2h_m[k, :, :], self.mass, axis=-1) / rho_mean + B)
