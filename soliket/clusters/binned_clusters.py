...
rhocrit0 = 2.7751973751261264e11 # [h2 msun Mpc-3] : computed using below
c_ms = 3e8                       # [m s-1]
Mpc = 3.08568025e22              # [m]
...
class BinnedClusterLikelihood(BinnedPoissonLikelihood):
    ...
    selection_func: Optional[str] = None
    ...
    def initialize(self):
        ...
        # mass bin - mass in units of Msun/h
    ...
    def _get_DAz(self, z):
        # angular diameter distance is in units of Mpc here
        # angular diameter distance in szcounts.f90 is in units of Mpc h-1
    ...
    def _get_dndlnm(self, z, pk_intp, **params_values_dict):
        ...
        rhom0 = rhocrit0 * om
        zarr = self.zarr
        marr = self.marr # in units of Msun/h
    ...
    def _get_dVdzdO(self, z):
        # in szcounts.f90 volume element is in units of h-3 Mpc3

        h = self.theory.get_param("H0") / 100.0
        Hz = self.theory.get_Hubble(z) * 1e3 / h
        dAz = self._get_DAz(z) * h

        return (c_ms/Hz)*(((1. + z)*dAz)**2.)
    ...
    def _get_M500c_from_M200m(self, M200m, z):
        ...
        def rho_crit(zz):
            ...
            return 3. / (8. * np.pi * GG) * np.power(H0 * Ehz(zz), 2.)
    ...
    def _get_integrated(self, pk_intp, **params_values_dict):
        ...
        surveydeg2 = self.skyfracs.sum() # in steradian
        ...
        nzarr = np.arange(0, self.zmax+0.2, 0.1)
            ...
            for ii in zs:
                ...
                for j in range(len(marr)):
                    if sel == "yes":
                        ...
                        sumzs[ii] += 0.5 * (intgr[ii,j] + intgr[ii+1,j]) * dlnm * (zz[ii+1] - zz[ii])
    ...
    def _get_integrated2D(self, pk_intp, **params_values_dict):
        ...
        for kk in range(Nq):
            ...
            for i in range(len(zarr)):
                ...
                if sel == "no": sum = sum/Nq
                delN2D[i,kk] = sum

            if sel == "yes": print(kk, delN2D[:,kk].sum())
            else: print(kk, delN2D[:,kk].sum()*Nq)
    ...
    def _get_y0(self, mass, z, **params_values_dict):
        ...
        A0 = params_values_dict["tenToA0"] # normalisation
        B0 = params_values_dict["B0"] # mass evolution
        C0 = params_values_dict["C0"] # redshift evolution
        bias = params_values_dict["bias_sz"] # mass bias

        Ez = self._get_Ez(z)
        Ez = Ez[:,None]
        h = self.theory.get_param("H0") / 100.0
        mb = mass * bias  # mass in units of Msun/h
        mpivot = 3e14 * h # making mpivot in units of Msun/h as well

        def theta(m):

            # from szcounts.f90 for Planck - theta500 in units of arcmin
            ...
            return ttstar*(m/3e14*(100./H0))**alpha_theta * Ez**(-2./3.) * (DAz/500.*(100./H0))**(-1.)

        def splQ(x):

            # interpolate from given file of Q(theta) - when perfectly matched Q = 1

            if single_tile == 'yes' or Q_opt == 'yes':
                tck = interpolate.splrep(self.tt500, self.Q)
                newQ = interpolate.splev(x, tck)
                #s = interpolate.InterpolatedUnivariateSpline(self.tt500, self.Q)
                #newQ = s(x)
            else:
                newQ = []
                i = 0
        ...
        def rel(m):
            ...
            # taken from Hasselfield 2013 (Page 5)

            t = -0.00848*(m*Ez)**(-0.585)
            return 1. + 3.79*t - 28.2*(t**2.)

        relf = self.rel_correction

        if relf == 'yes':
            y0 = A0 * (mb/mpivot)**(1. + B0) * (Ez**C0) * splQ(theta(mb)) * rel(mb/mpivot)
        else:
            y0 = A0 * (mb/mpivot)**(1. + B0) * (Ez**C0) * splQ(theta(mb))
