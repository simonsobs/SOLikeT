...
class KSZLikelihood(SZLikelihood):  # this is for GNFW model
    ...
    def _get_theory(self, **params_values):
        model_params = [
            params_values["gnfw_rho0"],
            params_values["gnfw_bt_ksz"],
...
class TSZLikelihood(SZLikelihood):  # this is for GNFW model
    def _get_data(self, **params_values):
        # Modify these lines as necessary. Usecols 2 for FFT, 4 for Hankel
        thta_arc, tsz_data = np.loadtxt(self.sz_data_file, usecols=(0, 4), unpack=True)
        tsz_data /= 3282.8 * 60.0**2  # units muK*sr
    ...
    def _get_theory(self, **params_values):
        model_params = [
            params_values["gnfw_P0"],
            params_values["gnfw_bt_tsz"],
...
class OBBLikelihood(SZLikelihood):  # OBB model, tSZ and kSZ together
    def _get_data(self, **params_values):
        thta_arc, ksz_data, tsz_data = np.loadtxt(
            self.sz_data_file, usecols=(0, 1, 2), unpack=True
        )
        ksz_data /= 3282.8 * 60.0**2
        tsz_data /= 3282.8 * 60.0**2
    ...
    def _get_theory(self, **params_values):
        model_params = [
            params_values["obb_Gamma"],
            params_values["obb_alpha_Nth"],
            params_values["obb_logE"],
            params_values["obb_Ak2h"],
