import numpy as np

from .config import mopc_datadir

def create_synthetic_beam(fwhm=1.4):
    """Returns beam of appropriate shape to use in mopc 
    """
    ...


"""from pixell.utils.beam_transform_to_profile"""


def beam_transform_to_profile(bl, theta, normalize=False):
    """Given the transform b(l) of a beam, evaluate its real space
    angular profile
    at the given radii theta."""
    bl = np.asarray(bl)
    l = np.arange(bl.size)
    x = np.cos(theta)
    a = bl * (2 * l + 1) / (4 * np.pi)
    profile = np.polynomial.legendre.legval(x, a)
    if normalize:
        profile /= np.sum(a)
    return profile


def read_beam(filename, normalize=True):
    """Reads the beam multipole info from file, returns real-space beam data
    """
    ell, b_ell = np.genfromtxt(filename, unpack=True)
    theta_arcmin = np.linspace(0, 20, 100)  # arcmin
    theta_rad = theta_arcmin / 60.0 * (np.pi / 180)  # radians
    b = beam_transform_to_profile(b_ell, theta_rad)

    if normalize:
        integ = np.trapz(b * 2 * np.pi * theta_rad, x=theta_rad)
        b /= integ

    return b


# beam_150_file = ...
# np.genfromtxt("/Users/samodeo/Desktop/map_coadd_S18/beam/beam_f150_daynight.txt")[:, 1]
# beam_090_file = ...
# np.genfromtxt("/Users/samodeo/Desktop/map_coadd_S18/beam/beam_f090_daynight.txt")[:, 1]
# theta_arcmin = np.linspace(0, 20, 100)  # arcmin
# theta_rad = theta_arcmin / 60.0 * (np.pi / 180)  # radians
# b150 = beam_transform_to_profile(beam_150_file, theta_rad)
# b090 = beam_transform_to_profile(beam_090_file, theta_rad)
# integ150 = np.trapz(b150 * 2 * np.pi * theta_rad, x=theta_rad)
# integ090 = np.trapz(b090 * 2 * np.pi * theta_rad, x=theta_rad)
# b150 /= integ150
# b090 /= integ090
# b150_n = beam_transform_to_profile(beam_150_file, theta_rad, normalize=True)
# b090_n = beam_transform_to_profile(beam_090_file, theta_rad, normalize=True)

b150 = read_beam(mopc_datadir.joinpath("beam_f150_daynight.txt"))
b090 = read_beam(mopc_datadir.joinpath("beam_f090_daynight.txt"))


def f_beam_150(tht):
    # b150 = read_beam("beam_f150_daynight.txt")

    theta_arcmin = np.linspace(0, 20, 100)
    theta_rad = theta_arcmin / 60.0 * (np.pi / 180)

    tht_in = theta_rad

    return np.interp(tht, tht_in, b150, period=np.pi)


def f_beam_090(tht):
    # b090 = read_beam("beam_f090_daynight.txt")

    theta_arcmin = np.linspace(0, 20, 100)
    theta_rad = theta_arcmin / 60.0 * (np.pi / 180)

    tht_in = theta_rad
    return np.interp(tht, tht_in, b090, period=np.pi)
