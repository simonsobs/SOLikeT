...
def rho_cz(z, provider):
    ...
    Ez2 = provider.get_param("Omega_m") * (1 + z) ** 3.0 + (
        1 - provider.get_param("Omega_m")
    )
...
def ComDist(z, provider):
    ...
    if O_tot < 1.0:
        ans = (
            Dh
            / np.sqrt(1.0 - O_tot)
            * np.sin(np.sqrt(1.0 - O_tot) * quad(ComInt, 0, z, args=provider)[0])
        )
    if O_tot > 1.0:
        ans = (
            Dh
            / np.sqrt(O_tot - 1.0)
            * np.sinh(np.sqrt(O_tot - 1.0) * quad(ComInt, 0, z, args=provider)[0])
        )
    return ans
