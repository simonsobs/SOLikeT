...
def test_ksz(request):
    info = {"params": {"gnfw_rho0":3.186441,
    ...
    thta_arc,ksz_gnfw,tsz_gnfw = np.loadtxt(os.path.join(request.config.rootdir, sz_data_file),usecols=(0,1,2),unpack=True)
    ...
    ksz_theory *= 3282.8 * 60.**2
    print("ksz_theory",ksz_theory)
    print("ksz_gnfw",ksz_gnfw)

    assert np.allclose(ksz_gnfw,ksz_theory,atol=1.e-1,rtol=0.1)
'''
