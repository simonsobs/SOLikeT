...
def update_covmat(data, Nsplits = 4):
    ...
    # We can now update covmat -> mean of covs
...
np.savetxt(f"{outdir}/covmat.txt", proposal_matrix @ proposal_matrix.T, header = "#" + " ".join(param_names))
...
for n in pbar:
    ...
    fg_vec = fgmarge._get_foreground_spectra(None, **param_dict, **fixed_params)
    ...
    new_point = current_point + covmat @ np.random.normal(loc = 0.0, scale = 1.0, size = current_point.shape)
    ...
    if accept:
        ...
        if burn_in > 0:
            ...
            last_update += 1
            if last_update >= update_covmat_every:
                print(f"Covmat update at {accepted+1:6d} accepted samples.")
                last_update = 0
                weights = accepted_samples[:,0]
                m = int(0.3 * sum(weights))
                Rminus1, new_covmat = update_covmat(accepted_samples[np.cumsum(weights) >= m,:])
                Rminus1_last = Rminus1
                
                if Rminus1 < Rminus1_best:
                    print(f"\tAccepted covmat.")
                    covmat = new_covmat
                    Rminus1_best = Rminus1
                    np.savetxt(f"{outdir}/covmat-{accepted+1:06d}.txt", covmat @ covmat.T, header = "#" + " ".join(param_names))
