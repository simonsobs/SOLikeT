...
class ForegroundMarginalizer(MFLike):
    ...
    def make_mapping_matrix(self, **params_nuisance):
        ...
        for m in self.spec_meta:
            l = m["leff"][m["leff"] <= self.lmax_extract.get(m["pol"], 0)]
            ...
            j = self.extract_zero[p] + np.where(np.in1d(self.lbins[p], l))[0]
            
            # Now we can populate the matrices.
            self.projection_matrix[i,j] = 1.0
            self.mapping_matrix[i,j] = 1.0 / (cal1 * cal2)
            self.mapping_ls[j] = (m["bpw"].weight.T @ np.arange(m["bpw"].weight.shape[0]))[m["leff"] <= self.lmax_extract[m["pol"]]]
        
        # Now we can build the extract & step matrix.
        self.extract_matrix = linalg.inv(self.mapping_matrix.T @ self.inv_cov @ self.mapping_matrix)
        self.step_matrix = linalg.cholesky(self.extract_matrix, lower = True)
    
    def extract(self, fg_vec):
        bb = self.mapping_matrix.T @ self.inv_cov @ (self.data_vec - fg_vec)
