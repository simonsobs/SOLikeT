...
for m in fgmarge.spec_meta:
    if m["t1"] == "LAT_145" and m["t2"] == "LAT_145":
        ...
        win = sacc.windows.BandpowerWindow(2 + np.arange(fgmarge.lmax_extract[p] + 1), win.weight[:fgmarge.lmax_extract[p]+1,:fgmarge.extract_bins[p]])
        
        t1, t2 = tracers[p]
        s.add_ell_cl(datatypes.get(p), t1, t2, cmb_ls[bin0:bin1], cmb_mean[bin0:bin1], window = win)
